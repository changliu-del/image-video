...
- Template list/detail HTTP cache headers stay short (`max-age=30`, `s-maxage=60`) because Admin can edit template media and prompt. The server-side base-array cache is cleared by Admin template writes. Public R2-backed template media (`assets.storage_key` under `templates/`) should be returned as direct `R2_PUBLIC_BASE_URL + storage_key` CDN URLs so browsers do not proxy every public thumbnail/preview through Vercel Functions. `/api/template-media/{assetId}` remains the range-capable compatibility/fallback route for stored snapshots and explicit `external/` storage keys.
- Uploaded user assets and generated outputs should be displayed through `/api/asset-media/{assetId}`. Workbenches should not render raw `assets.public_url` for private media because that URL is storage metadata and may not be browser-readable.
