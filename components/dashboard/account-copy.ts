...
export const billingCopy: Record<DashboardLocale, BillingCopy> = {
  ...
  zh: {
    ...
    title: '按你的创作负载选择订阅计划',
    intro:
      '这一阶段支付统一走 mock，不会真实扣款。订阅会立即发放当月算力值，之后接入真实支付时仍复用同一套 price id。',
    buyExtra: '购买额外算力值',
    balance: '余额',
    balanceHint: '可用算力值',
    currentPlan: '当前计划',
    ...
    mockTitle: 'Mock 支付模式',
    mockBody:
      'Checkout 现在不会真实扣钱，只会更新账户计划、增加算力值并写入账本，方便完整验证工作台链路。',
    mockSuccess: 'Mock 订阅已开通，算力值已发放。',
    mockCanceled: 'Mock 订阅已取消，剩余算力值仍可继续使用。',
    loadError: '账户状态加载失败，订阅计划仍可继续选择。',
    retry: '重试',
    manageBilling: '管理账单',
    cancelPlan: '取消 mock 计划',
    intervalTabs: { month: '月付', year: '年付' },
    intervalLabels: { month: '月付计划', year: '年付计划' },
    intervalHint: '年付计划仍按月刷新算力值，但折算月费更低。',
    recommended: '推荐',
    current: '当前',
...
export const creditsCopy: Record<DashboardLocale, CreditsCopy> = {
  ...
  zh: {
    badge: '算力钱包',
    title: '当活动需要更多余量时，直接购买额外算力值',
    intro:
      '单次购买的算力值独立于订阅月额度。mock 阶段会立即到账，并进入生成任务共用的可用余额。',
    balance: '可用余额',
    ...
    packages: {
      starter: {
        description: '适合 prompt 测试和小规模商品实验的轻量补充。',
        features: ['100 购买算力值', 'mock 模式下不会过期', '约可生成 10 条标准 5 秒视频'],
      },
      creator: {
        description: '适合每周一批商品图和视频素材的额外余量。',
        features: ['500 购买算力值', 'mock 模式下不会过期', '约可生成 50 条标准 5 秒视频'],
      },
      scale: {
        description: '适合目录批量生产和活动测试的更大储备。',
        features: ['1500 购买算力值', 'mock 模式下不会过期', '约可生成 150 条标准 5 秒视频'],
      },
    },
  },
};
