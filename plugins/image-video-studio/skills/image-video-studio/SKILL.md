...
- Treat media URLs as an app-level contract, not a raw R2 contract: public template thumbnails/previews should prefer direct CDN URLs built from `R2_PUBLIC_BASE_URL + assets.storage_key` when the asset is uploaded and under `templates/`, with `/api/template-media/{assetId}` kept as the compatibility/fallback route for historical snapshots, range checks, and explicit `external/` media. User uploads and generated outputs should flow through `/api/asset-media/{assetId}` because they are private. `assets.public_url` is storage metadata and should not be returned to frontend workbenches/Admin previews as the display URL.
