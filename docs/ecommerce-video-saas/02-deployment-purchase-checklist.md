...
| 生成模型 | Bailian DashScope / Wanxiang API | 按供应商账号/合同入口 | 图生视频、商品图、试衣都使用百炼 DashScope API Key | 否 |
...
```text
确认供应商账号或合同入口
开通百炼 DashScope API Key
确认图生视频、商品图、智能试衣所需模型已开通
用公开图片做 DashScope submit/query 联调
```
...
```text
DASHSCOPE_API_KEY 只能存在服务端环境变量中。
